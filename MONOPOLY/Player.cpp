#include "Player.hpp"

const int BOARD_WIDTH = 800;
const int BOARD_HEIGHT = 800;
const double Slots_SIZE = 73; // Adjusted for 11 Slots
const float TOKEN_RADIUS = 10.0f; // Adjust size as needed
// Create a map of string colors to sf::Color with 3 additional colors
std::map<std::string, sf::Color> colorMap = {
        {"blue", sf::Color::Blue},
        {"black", sf::Color::Black},
        {"green", sf::Color::Green},
        {"yellow", sf::Color::Yellow},
        {"purple", sf::Color(128, 0, 128)}, 
        {"orange", sf::Color(255, 165, 0)}, 
        {"pink", sf::Color(255, 192, 203)}, 
        {"cyan", sf::Color::Cyan} 
};
Player::Player(std::string name,std::string color,sf::RenderWindow &window,int serialNum): currentSlots(Board::getSlots()[0])
{
    this->name = name;
    this->color = color;
    this->money = 1500;
    this->currentIndex = 0;
    this->serialNum = serialNum;
    initializePlayerToken(window);
}
std::string Player::getColor()
{
    return this->color;
}
std::string Player::getName()
{
    return this->name;
}

// New function to draw the purchase button on the screen
void Player::drawBuyButton(sf::RenderWindow &window)
{
    // Load font for the text
    sf::Font font;
    font.loadFromFile("arial.ttf");
    // Set up the roll dice button
    buyButton = Button(100, 50, "yes", font, sf::Color::Yellow, [](){});
    buyButton.setPosition(BOARD_WIDTH / 2 - 50, BOARD_HEIGHT / 2 + 240); // Adjust position
    buyButton.draw(window);
}

bool Player::handleBuyButtonClick(sf::RenderWindow &window)
{
    if(currentSlots->ownerColor == "None" || checkIfCanBuildHouse() || checkIfCanBuildHotel())
    {
        int moneyAfter;
        if(currentSlots->ownerColor == "None")
        {
            if(getCurrentSlots()->getKindAsString() == "Street")
            {
                Street *street = dynamic_cast<Street *>(currentSlots->getKind());
                moneyAfter = street->buyStreet(money);
            }
            else if(getCurrentSlots()->getKindAsString() == "Train")
            {
                Train *train = dynamic_cast<Train *>(currentSlots->getKind());
                moneyAfter = train->buyTrain(money);
                if(moneyAfter > 0)
                {
                    numberOfTrains++;
                }
            }
            else
            {
                Utility *utility = dynamic_cast<Utility *>(currentSlots->getKind());
                moneyAfter = utility->buyUtility(money);
            }
            if (moneyAfter > 0)
            {
                sizeMarkers++;
                sf::CircleShape newMarker(5.0f); // Create a small circle for ownership
                newMarker.setFillColor(token.getFillColor()); // Set the marker's color to match the player's token color
                // Adjust the position of the marker (near the player's token)
                newMarker.setPosition(token.getPosition().x + 15, token.getPosition().y + 15);
                // Add the new marker to the ownershipMarkers vector
                ownershipMarkers.push_back(newMarker);
            }
        }
        else
        {
            Street* street = dynamic_cast<Street*>(currentSlots->getKind());
            moneyAfter = street->buildHouse(money, currentSlots->numberOfHouses);
            if (moneyAfter > 0)
            {
                currentSlots->numberOfHouses++;
                sizeMarkers++;
                sf::CircleShape newMarker(6.0f); // Create a small circle for ownership
                newMarker.setFillColor(token.getFillColor()); // Set the marker's color to match the player's token color
                // Adjust the position of the marker (near the player's token)
                newMarker.setPosition(token.getPosition().x+10*(currentSlots->numberOfHouses), token.getPosition().y-3*(currentSlots->numberOfHouses));
                // Add the new marker to the ownershipMarkers vector
                if(currentSlots->numberOfHouses == 5)
                {
                    newMarker.setRadius(10.0f); // Create a small circle for ownership
                    newMarker.setPosition(token.getPosition().x, token.getPosition().y);

                }
                ownershipMarkers.push_back(newMarker);
            }
        }
        if(moneyAfter > 0)
        {
            money = moneyAfter;
            ownedProperties.push_back(*currentSlots);
            // For example, drawing a small circle on the slots to indicate ownership
            currentSlots->ownerColor = color;
            nowBuy = true;
            return true;
        }
    }
    return false;
}
void Player::initializePlayerToken(sf::RenderWindow &window)
{
    int i = this->serialNum;
    token.setRadius(TOKEN_RADIUS); // Set the radius of the token
    // Find the color based on the player's color string
    if (colorMap.find(color) != colorMap.end())
    {
        token.setFillColor(colorMap[color]); // Set the token color
    }
    else
    {
        token.setFillColor(sf::Color::Red); // Default to white if color not found
    }
    token.setPosition(BOARD_WIDTH  - (Slots_SIZE/1.5) , BOARD_HEIGHT - (Slots_SIZE)+12*(i+1)-10); // Adjust positioning
    // Load font for the text
    sf::Font font;
    font.loadFromFile("arial.ttf");
    // Initialize the text for player name and money
    playerInfo.setFont(font); // Set the font
    playerInfo.setCharacterSize(14); // Text size
    playerInfo.setFillColor(token.getFillColor()); // Text color same as token
    playerInfo.setString(name + ": $" + std::to_string(money)); // Set the string (name and money)
    // Update the player info (name and money) position and string
    playerInfo.setPosition(BOARD_WIDTH / 2 - 50, BOARD_HEIGHT / 2 + 35*(i+1));
    playerInfo.setString(name + ": $" + std::to_string(money));
    window.draw(token);
    window.draw(playerInfo);

}

Slots* Player::getCurrentSlots()
{
    return currentSlots;
}
void Player::drawTokenAndInfo(sf::RenderWindow &window)
{
    window.draw(token);
    // Load font for the text
    sf::Font font;
    font.loadFromFile("arial.ttf");
    // Initialize the text for player name and money
    playerInfo.setFont(font); // Set the font
    playerInfo.setCharacterSize(14); // Text size
    playerInfo.setString(name + ": $" + std::to_string(money)); // Set the string (name and money)
    // Update the player info (name and money) position and string
    playerInfo.setFillColor(token.getFillColor()); // Text color same as token
    playerInfo.setPosition(BOARD_WIDTH / 2 - 50, BOARD_HEIGHT / 2 + 35*(serialNum+1));
    playerInfo.setString(name + ": $" + std::to_string(money));
    window.draw(playerInfo);
    for(int i = 0; i < sizeMarkers; i++)
    {
        window.draw(ownershipMarkers[i]); // Draw the marker on the window
    }
    // Check if the current slots is purchasable
    if (currentSlots->isPurchasable() && !nowBuy && hisTurn && ((currentSlots->ownerColor == color && (checkIfCanBuildHouse() || checkIfCanBuildHotel())) || currentSlots->ownerColor == "None"))  // Dummy function to check if the slots can be bought
    {
        if(currentSlots->ownerColor == "None")
        {
            playerInfo.setString("do you want to buy the property?");
        }
        else
        {
            if(currentSlots->numberOfHouses < 4)
            {
                playerInfo.setString("do you want to buy a house?");
            }
            if(currentSlots->numberOfHouses == 4)
            {
                playerInfo.setString("do you want to buy a hotel?");
            }
        }
        playerInfo.setPosition(BOARD_WIDTH / 2 - 100, BOARD_HEIGHT / 2 + 200);
        playerInfo.setFillColor(sf::Color::Black);
        window.draw(playerInfo);
        drawBuyButton(window); // Draw the button to buy the property
    }
}
void Player::setCurrentSlots(int numToMove ,sf::RenderWindow &window)
{
    int i = this->serialNum;
    if(currentIndex+numToMove >= Board::getSlotss().size())
    {
        money += 200;
        std::cout << name << " passed Go and received 200!" << std::endl;
        this->currentSlots = Board::getSlotss()[currentIndex+numToMove-Board::getSlotss().size()];
        currentIndex = currentIndex+numToMove-Board::getSlotss().size();
    }
    else
    {
        this->currentSlots = Board::getSlotss()[currentIndex+numToMove];
        currentIndex += numToMove;
    }
    // Set the slots's position based on its index (i)
    if (currentIndex < 11)
    {
        token.setPosition(BOARD_WIDTH +10- (currentIndex + 1) * Slots_SIZE+3, BOARD_HEIGHT +10- Slots_SIZE+5*(i+1));
    }
    else if (currentIndex < 20)
    {
        token.setPosition(20, BOARD_HEIGHT - (currentIndex - 9) * Slots_SIZE+5*(i+1));
    }
    else if (currentIndex < 31)
    {
        token.setPosition((currentIndex - 20) * Slots_SIZE+10, 5*(i+1));
    }
    else
    {
        token.setPosition(BOARD_WIDTH - Slots_SIZE+3, (currentIndex- 30) * Slots_SIZE+5*(i+1));
    }
    if (currentSlots->isPurchasable())  // Dummy function to check if the slots can be bought
    {
        if(currentSlots->ownerColor == "None" || checkIfCanBuildHouse() || checkIfCanBuildHotel())
        {
            drawBuyButton(window); // Draw the button to buy the property
            nowBuy = false;
        }
    }
}

bool Player::checkIfCanBuildHouse()
{
    if(currentSlots->ownerColor == color && currentSlots->getKindAsString() == "Street" && currentSlots->numberOfHouses < 4)
    {
        for(int i = currentSlots->getIndex()+1; i < currentSlots->getIndex()+4 && i < Board::getSlotss().size(); i++)
        {
            if(Board::getSlotss()[i]->getColor() == currentSlots->getColor())
            {
                if(Board::getSlotss()[i]->ownerColor != color || Board::getSlotss()[i]->numberOfHouses < currentSlots->numberOfHouses)
                {
                    return false;
                }
            }
        }
        for(int i = currentSlots->getIndex()-1; i > currentSlots->getIndex()-4 && i > 0; i--)
        {
            if(Board::getSlotss()[i]->getColor() == currentSlots->getColor())
            {
                if(Board::getSlotss()[i]->ownerColor != color || Board::getSlotss()[i]->numberOfHouses < currentSlots->numberOfHouses)
                {
                    return false;
                }
            }
        }
        return true;
    }
    return false;
}
bool Player::checkIfCanBuildHotel()
{
    if(currentSlots->ownerColor == color && currentSlots->getKindAsString() == "Street" && currentSlots->numberOfHouses == 4)
    {
        return true;
    }
    return false;
}